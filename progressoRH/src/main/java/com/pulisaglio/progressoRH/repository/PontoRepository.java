...
public class PontoRepository {
    ...
    public void save(Ponto ponto) {
        final String sql = "INSERT INTO pontos (tipo, data_hora, contrato_id) VALUES (?, ?, ?)";
    ...
    public Optional<Ponto> findById(Integer id) {
        final String sql = "SELECT id_ponto, tipo, data_hora, contrato_id FROM pontos WHERE id_ponto = ?";
    ...
    public List<Ponto> findAll() {
        final String sql = "SELECT id_ponto, tipo, data_hora, contrato_id FROM pontos";
        return jdbcTemplate.query(sql, ROW_MAPPER);
    }

    public List<Ponto> findByContratoId(Integer contratoId) {
        final String sql = "SELECT id_ponto, tipo, data_hora, contrato_id FROM pontos WHERE contrato_id = ?";
        return jdbcTemplate.query(sql, ROW_MAPPER, contratoId);
    }

    public void update(Ponto ponto) {
        final String sql = "UPDATE pontos SET tipo = ?, data_hora = ?, contrato_id = ? WHERE id_ponto = ?";
        jdbcTemplate.update(sql,
                ponto.getTipo(),
                Timestamp.valueOf(ponto.getDataHora()),
                ponto.getContratoId(),
